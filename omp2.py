...
def omp2()
	...
	if esx[6] == 1: oxy =  np.transpose(oxy[index])
	if esx[7] == 1: ph  =  np.transpose(ph[index])
	if esx[8] == 1: ni  =  np.transpose(ni[index])
	if esx[9] == 1: si  =  np.transpose(si[index])
    del index
...
for k=1:length(lat);
	# selecting the correct parameters
     ...
     t_dat	= ptemp(k); 
     s_dat	=   sal(k); 
     if esx(6) == 1 oxy_dat	=   oxy(k)
     if esx(7) == 1  ph_dat	=    ph(k)
     if esx(8) == 1  ni_dat	=    ni(k)
	 if esx(9) == 1  si_dat    =    si(k)
     if exist('pdens') == 1  pden_dat	= pdens(k)
	 if esx(10) == 1 pv_dat = pvort(k)
     kon=1;
...
print '  salinity'
if esx(6)   == 1 print '  oxygen'
if esx(7)   == 1 print '  phosphate'
if esx(8)   == 1 print '  nitrate'
if esx(9)   == 1 print '  silicate'
if esx(10)   == 1 print '  potential vorticity'
...
if j == 'y':
	drswitch('Output is stored in')
		...
		if length(incontrol) > 0 vname = incontrol
		incontrol = vname
		lv = length(vname)+1
switch(OMP)
case 'ext'	
...
otherwise		
   vname = [vname '  nr_of_wm tit_index A err esx lat long press pdens']
		...
		if esx(4)  == 1, vname = [vname ' sal']
		if esx(5)  == 1, vname = [vname ' ptemp']
		if esx(6)  == 1, vname = [vname ' oxy']
		if esx(7)  == 1, vname = [vname ' ph']
		if esx(8)  == 1, vname = [vname ' ni']
		if esx(9)  == 1, vname = [vname ' si']
		if esx(10) == 1, vname = [vname ' pvort']
		
		sout = sprintf('save %s',vname)
		eval(sout)
		print '  '
		print ['File ' incontrol ' created and saved as: '  vname(1:lv) '.mat']
		print [' in:  ' pwd]
