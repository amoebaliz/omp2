...
# FIGURE SET UP
f, ax = plt.subplots(1, 4,figsize=(12,5),sharey='row')
...
for wt in range(3):
    ...
    for nv in range(4):    
       ...
       #ax[nv].plot(VAR,PTEMP,'b+')
       #ax[nv].plot(np.polyval(coeff,PTEMP),PTEMP,'r')
       tms, = ax[nv].plot(np.array(CCS_sw_dict[sw_vars[nv+1]]),np.array(CCS_sw_dict['PTEMP']),'.',markersize=3,label=wms[wt])
       print '-------------------'
...
f.text(0.08, 0.5, 'Temperature ($^\circ$C)', ha='center', va='center', rotation='vertical',fontsize=13)

ax[0].set_xlabel('Salinity (g/kg)',fontsize=12)
ax[1].set_xlabel('Oxygen (ml/L)',fontsize=12)
ax[2].set_xlabel('Silicate (umol/L)',fontsize=12)
ax[3].set_xlabel('Phosphate (umol/L)',fontsize=12)

#ax3[1,0].set_yticks(ytick_r1[wt])
#ax3[0,0].set_yticks(ytick_r2[wt])
lgnd = plt.legend(loc='center',bbox_to_anchor=(-1.3, 1.1),ncol=3,markerscale=10,fontsize=10)
